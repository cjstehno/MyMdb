...
class MymdbDbRealm {
    ...
    def authenticate(authToken) {
        ...
        // exception.
        def user = MymdbUser.findByUsername(username)
        if (!user) {
        ...
        // in the database.
        def account = new SimpleAccount(username, user.passwordHash, "MymdbDbRealm")
    ...
    def hasRole(principal, roleName) {
        def roles = MymdbUser.withCriteria {
            roles {
                eq("name", roleName)
            }
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = MymdbUser.findByUsername(principal)
        ...
        def results = MymdbUser.executeQuery("select distinct p from MymdbUser as user join user.roles as role join role.permissions as p where user.username = '$principal'")
